...
//                        questionDAO.(question);
            ...
            response.sendRedirect(request.getContextPath() + "/manage-question?action=view&id=" + lessonId);
            
        } catch (Exception e) {
            request.getSession().setAttribute("toastMessage", "Error updating quiz: " + e.getMessage());
            request.getSession().setAttribute("toastType", "error");
            response.sendRedirect(request.getContextPath() + "/manage-question?action=edit&id=" + lessonId);
        }
    }
...
//                questionDAO.deleteByLessonQuizId(quiz.getId());
    ...
    private void saveQuestion(HttpServletRequest request, HttpServletResponse response)
        ...
        if (quizId == null || quizId.isEmpty() || questionText == null || questionText.isEmpty()) {
            ...
            request.getSession().setAttribute("toastType", "error");
            response.sendRedirect(request.getContextPath() + "/manage-question?action=edit&id=" + quizId);
            return;
        }
        ...
        try {
            ...
            request.getSession().setAttribute("toastType", "error");
            response.sendRedirect(request.getContextPath() + "/manage-question?action=editQuestion&quizId=" + quizId + "&questionId=" + questionId);
        }
    }
